...
const tileBeep = new URL(
  "../sound-effects/memorytile-beep.mp3",
  import.meta.url
);

const applause = new URL(
  "../sound-effects/applause-sound.mp3",
  import.meta.url
);
...
const amountCustomDifficulty = document.getElementById(
  "amount-custom-difficulty"
);
...
function getCustomDifficultyInput() {
  ...
  if (
    2 <= amountCustomDifficulty.value &&
    amountCustomDifficulty.value <= 100 &&
    581 < screenWidth
  ) {
  ...
  if (
    2 <= amountCustomDifficulty.value &&
    amountCustomDifficulty.value <= 50 &&
    581 >= screenWidth
  ) {
    numberOfTiles = amountCustomDifficulty.value;
    messageBoxCustom.innerHTML = "";
...
function createMemoryTilesHtml() {
  for (let i = 1; i <= numberOfTiles; ++i) {
    ...
    if (numberOfTiles <= 21) {
      createMemoryTile.classList.add(
        "memory-block",
        "memory-block-height--10",
        `memory-block--${i}`
      );
    }
...
function winCheck() {
  ...
  if (playerNumbersSequence.length === generatedNumbersSequence.length) {
    ...
    /* compare arrays */
    if (
      JSON.stringify(playerNumbersSequence) ===
      JSON.stringify(generatedNumbersSequence)
    ) {
...
function styleColumnsGameArea() {
  ...
  for (let i = 2; i < 7; i++) {
    if (numberOfTiles <= i * 3 + 3 && i * 3 < numberOfTiles) {
      memoryBlockArea.className = `memory-block-area margin--${
        40 - i * 5
      } gap--3 grid-col`;
